...
import { formatTimeFromFirestoreLike, toMillisFromFirestoreLike } from '../../utils/formatters'
...
const ChatBox = ({ roomId, userId, senderName }) => {
    ...
    return (
        <div className='absolute right-6 bottom-6 w-72 h-44 flex flex-col mask-t-from-50%'>
            <div
            ...
            >
                {mergedMessages.map((message) => {
                    ...
                    return (
                        <div
                            key={message.uid}
                            className={`p-1 whitespace-pre-wrap break-words bg-neutral0/5 flex justify-between gap-2 ${message.pending ? 'text-neutral1' : ''}`}
                        >
                            <div>
            ...
            <div>
                <input
                    ...
                    className='w-full bg-neutral0/5 p-1 text-[10px] outline-none rounded-b-md'
                />
            </div>
        </div>
    )
