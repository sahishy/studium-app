...
const BLANK_LINE_HTML = '<span aria-label="blank">_____</span>'

const normalizeBlankMarkers = (value) => String(value ?? '')
    // SAT source data represents blanks as an aria-hidden run of underscores
    // followed by screen-reader-only text such as "blank".
    .replace(/<span\b[^>]*>\s*_+\s*<\/span>\s*<span\b[^>]*>\s*blank\s*<\/span>/gi, BLANK_LINE_HTML)
    // Handle the same marker when it arrives as plain text.
    .replace(/_+\s*blank\b/gi, BLANK_LINE_HTML)
...
const normalizeDsatQuestion = ([entryKey, entry], i) => {
    ...
    const baseQuestion = {
        id: questionId,
        body: content?.body == null ? null : normalizeBlankMarkers(content.body),
        prompt: normalizeBlankMarkers(getQuestionPrompt(content)),
        paragraph: getQuestionParagraph(content) == null ? null : normalizeBlankMarkers(getQuestionParagraph(content)),
